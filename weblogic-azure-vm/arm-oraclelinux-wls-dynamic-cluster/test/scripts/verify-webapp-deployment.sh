...
curl --verbose ${appGatewayURL}weblogic-cafe/rest/coffees
response=$(curl ${CURL_RETRY_PARMS}  --write-out '%{http_code}' --silent --output /dev/null ${appGatewayURL}weblogic-cafe/rest/coffees)
echo "$response"
