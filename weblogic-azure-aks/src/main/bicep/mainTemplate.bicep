...
var const_acrName = (createACR) ? 'acrwlsaks${const_globalResourceNameSufix}' : acrName
...
module autoGeneratedSSLCert './modules/_azure-resoruces/_autoGeneratedPfxCertandSaveinKeyvault.bicep' = if (enableAppGWIngress && appGatewayCertificateOption == const_appGatewaySSLCertOptionGenerateCert) {
  name: 'auto-generated-ssl-cert-for-app-gateway'
  params: {
    _globalResourceNameSufix: const_globalResourceNameSufix
    identity: obj_uamiForDeploymentScript
    keyVaultName: 'wlskv${const_globalResourceNameSufix}'
    location: location
...
module appgatewayDeployment 'modules/_appGateway.bicep' = if (enableAppGWIngress) {
  ...
  params: {
    ...
    azCliVersion: const_azcliVersion
    autoGeneratedSSLCertKeyVaultName: autoGeneratedSSLCert.outputs.keyVaultName
    autoGeneratedSSLCertSecretName: autoGeneratedSSLCert.outputs.secretName
